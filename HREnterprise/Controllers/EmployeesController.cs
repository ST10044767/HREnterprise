    ...
    public class EmployeesController : Controller
    {
        ...
        public async Task<IActionResult> Create(Employee employee, IFormFile ProfileImage)
        {
            if (!ModelState.IsValid)
            {
                ...
                {
                    var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/images", ProfileImage.FileName);
                    using (var stream = new FileStream(path, FileMode.Create))
                    {
                        await ProfileImage.CopyToAsync(stream);
                    }
                    employee.ProfileImagePath = "/images/" + ProfileImage.FileName;
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,FullName,Email,Department,Position,DateHired,ProfileImagePath")] Employee employee)
        {
            if (id != employee.Id)
            {
                return NotFound();
